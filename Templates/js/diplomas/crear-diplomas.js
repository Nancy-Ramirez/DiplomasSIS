...
// Inicializar el canvas y el display de tamaño
...
function renderizarDiploma() {
    ...
    // Líneas de ayuda (solo se dibujan mientras se mueve un componente)
    if (arrastrando && seleccionado !== null && seleccionado !== "border") {
        const [type, index] = seleccionado.split("-");
        let centerX, centerY;

        if (type === "text") {
            const item = diplomaData.text[index];
            const maxWidth = canvas.width - 60;
            const lineHeight = item.font.size * 1.2;
            const lines = wrapText(ctx, item.content, item.position.x, item.position.y, maxWidth, lineHeight, item.alignment || "left");
            const textWidth = Math.max(...lines.map((line) => ctx.measureText(line.text).width));
            const textHeight = lines.length * lineHeight;
            centerX = item.position.x + textWidth / 2;
            centerY = item.position.y + textHeight / 2 - item.font.size;
        } else if (type === "image") {
            const item = diplomaData.images[index];
            centerX = item.position.x + item.size.width / 2;
            centerY = item.position.y + item.size.height / 2;
        } else if (type === "line") {
            const item = diplomaData.lines[index];
            centerX = (item.start.x + item.end.x) / 2;
            centerY = (item.start.y + item.end.y) / 2;
        }

        // Dibujar líneas de ayuda horizontales y verticales
        ctx.strokeStyle = "rgba(150, 150, 150, 0.8)"; // Gris claro con algo de transparencia
        ctx.lineWidth = 0.5; // Muy delgada para que no estorbe
        ctx.setLineDash([5, 5]); // Líneas discontinuas para que sean más suaves visualmente
        ...
        // Restaurar el estilo de línea para que no afecte otros elementos
        ctx.setLineDash([]);
    }
}
...
function vincularEventosEditor() {
    ...
    document.getElementById("saveDiploma").addEventListener("click", guardarDiploma);
    document.getElementById("startCreation").addEventListener("click", () => {
        renderizarDiploma();
...
function removeBackgroundImage() {
    if (diplomaData.canvas.background.type === "image") {
        delete diplomaData.canvas.background.image;
        diplomaData.canvas.background.type = "solid";
        diplomaData.canvas.background.color = "#ffffff";
        bgOpacity.value = 1;
        renderizarDiploma();
        actualizarEditorDiploma();
    }
...
async function guardarDiploma() {
    try {
        ...
        if (response.ok) {
            alert("¡Diploma creado y guardado exitosamente!");
        } else {
...
canvas.addEventListener("mouseup", () => {
    arrastrando = false;
    renderizarDiploma(); // Volver a renderizar para ocultar las líneas de ayuda
});
